...
spot_settings = {
    "初級：旧・Ｋトンネル": {
        "threshold": 0.3, 
        "required_time": 5, 
        "enemy_img": "p1back.png?text=Empty_Tunnel",
        "description": "比較的安全なスポット。小声でレポートを続けろ。",
        "outimg": "p1front.png?text=Empty_Tunnel"
    },
    "中級：○○病院跡": {
        "threshold": 0.15, 
        "required_time": 10, 
        "enemy_img": "p2back.png?text=Hospital_Shadow",
        "description": "何かの気配を感じる... 音を立てれば、即座に見つかるぞ。",
        "outimg": "p2front.png?text=Hospital_Shadow"
    },
    "上級：呪われたＳ家": {
        "threshold": 0.05, 
        "required_time": 15, 
        "enemy_img": "enemy_back.png?text=Cursed_Doll",
        "description": "最恐のスポット。息を殺せ。物音一つ立てるな。",
        "outimg": "enemy_front.png?text=Cursed_Doll"
    }
}
...
# --- 結果画面 ---
    ...
    with col1:
        if max_vol > conf['threshold']:
            ...
            # クリア演出
            st.image("enemy_clear.png?text=SAFE_RETURN", caption="生還")
